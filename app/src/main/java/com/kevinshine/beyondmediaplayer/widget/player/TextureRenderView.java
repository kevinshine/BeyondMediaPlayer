...
public class TextureRenderView extends TextureView {
    ...
    public void bindMediaPlayer(IMediaPlayer mp){
        if (mp == null)
            return;
        SurfaceTexture surfaceTexture = mSurfaceCallback.mSurfaceTexture;

        if ((Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) &&
                (mp instanceof TextureMediaPlayer)) {
            TextureMediaPlayer texturePlayer = (TextureMediaPlayer) mp;
            mSurfaceCallback.setOwnSurfaceTexture(false);

            SurfaceTexture oldSurfaceTexture = texturePlayer.getSurfaceTexture();
            if (oldSurfaceTexture != null) {
                setSurfaceTexture(oldSurfaceTexture);
            } else {
                texturePlayer.setSurfaceTexture(surfaceTexture);
                texturePlayer.setSurfaceTextureHost(mSurfaceCallback);
            }
        } else {
    ...
    private static final class SurfaceCallback implements SurfaceTextureListener, ISurfaceTextureHost {
        ...
        public void addRenderCallback(@NonNull IRenderCallback callback) {
            ...
            TextureRenderView renderView = mWeakRenderView.get();
            if (renderView == null) return;

            if (mSurfaceTexture != null) {
                callback.onSurfaceCreated(renderView, mWidth, mHeight);
            }

            if (mIsFormatChanged) {
                callback.onSurfaceChanged(renderView, 0, mWidth, mHeight);
            }
        }
        ...
        public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
            ...
            for (IRenderCallback renderCallback : mRenderCallbackMap.keySet()) {
                renderCallback.onSurfaceCreated(mWeakRenderView.get(), 0, 0);
            }
        }
        ...
        public void onSurfaceTextureSizeChanged(SurfaceTexture surface, int width, int height) {
            ...
            for (IRenderCallback renderCallback : mRenderCallbackMap.keySet()) {
                renderCallback.onSurfaceChanged(mWeakRenderView.get(), 0, width, height);
            }
        }
        ...
        public boolean onSurfaceTextureDestroyed(SurfaceTexture surface) {
            ...
            for (IRenderCallback renderCallback : mRenderCallbackMap.keySet()) {
                renderCallback.onSurfaceDestroyed(mWeakRenderView.get());
            }

...
extern int updateCurrentIndex(int index);
extern void getNextPlaylistItemIndexAndPath(int* index,char* path);
    ...
    jobjectArray keys, jobjectArray values)
{
    ...
    ALOGV("armv7a_IjkMediaPlayer_setDataSourceAndHeaders: path %s", c_path);
...
static void
IjkMediaPlayer_updateCurrentIndex(JNIEnv *env, jobject thiz, jint index)
{
    int i = index;
...
static void
IjkMediaPlayer_playNext(JNIEnv *env, jobject thiz)
{
    IjkMediaPlayer_reset(env,thiz);

    MPTRACE("%s\n", __func__);

    IjkMediaPlayer *mp = jni_get_media_player(env, thiz);
    JNI_CHECK_GOTO(mp, env, "java/lang/IllegalStateException", "mpjni: playNext: null mp", LABEL_RETURN);

    int retval = 0;
    int nextIndex = 0;
    char c_path[256];
    getNextPlaylistItemIndexAndPath(&nextIndex,c_path);

    ALOGV("%s: nextIndex %i",__func__, nextIndex);
    ALOGV("%s: setDataSource: %s", __func__, c_path);
...
extern jobjectArray PlaylistManager_getPlaylist(JNIEnv *env);

extern void PlaylistManager_addPlaylistItem(JNIEnv *env,jobject thiz,jobject mediaInfo);

extern void PlaylistManager_clearPlaylist(JNIEnv *env);
// ----------------------------------------------------------------------------
...
static JNINativeMethod g_methods[] = {
    ...
    { "updateCurrentIndex",     "(I)V",                     (void *) IjkMediaPlayer_updateCurrentIndex },
    { "playNext",               "()V",                      (void *) IjkMediaPlayer_playNext },

};

static JNINativeMethod playlist_methods[] = {
        { "getPlaylist",        "()[Ltv/danmaku/ijk/media/player/MediaInfo;",   (void *) PlaylistManager_getPlaylist },
        { "addPlaylistItem",        "(Ltv/danmaku/ijk/media/player/MediaInfo;)V",   (void *) PlaylistManager_addPlaylistItem },
        { "clearPlaylist",        "()V",                                        (void *) PlaylistManager_clearPlaylist },
};
...
jint JNI_OnLoad(JavaVM *vm, void *reserved)
{
    ...
    // Register playlist native methods
    if (!registerNativeMethods(env, JNI_CLASS_PLAYLIST_MANAGER, playlist_methods, NELEM(playlist_methods))){
        return -1;
    }

...
public class FloatingView extends FrameLayout implements ViewTreeObserver.OnPreDrawListener {
    ...
    public FloatingView(final Context context) {
        ...
        mMoveDirection = MOVE_DIRECTION_DEFAULT;
    ...
    public boolean onPreDraw() {
        ...
        // 画面端に移動しない場合は指定座標に移動
        if (mMoveDirection == MOVE_DIRECTION_NONE) {
    ...
    private void updateViewLayout() {
        ...
        if (oldScreenWidth != newScreenWidth || oldScreenHeight != newScreenHeight) {
            ...
            if (mMoveDirection == MOVE_DIRECTION_DEFAULT) {
                // 右半分にある場合
            ...
            // 左端に移動
            else if (mMoveDirection == MOVE_DIRECTION_LEFT) {
                mParams.x = mPositionLimitRect.left;
            }
            // 右端に移動
            else if (mMoveDirection == MOVE_DIRECTION_RIGHT) {
                mParams.x = mPositionLimitRect.right;
            }
